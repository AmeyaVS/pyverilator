...
class Collection:
    ...
    def __repr__(self):
        ...
        if truncated:
            ...
            keys_to_show.extend(
                [self._item_dict_keys[i] for i in range(num_items - 10, num_items)]
            )
        else:
        ...
        for i in range(len(items_to_show)):
            ...
            if truncated and i == 10:
                ret += f"    ... {num_items} objects in total ...\n"
            fmt_string = (
                "{:<"
                + str(column_one_width)
                + "}  {:<"
                + str(column_two_width)
                + "}  {:>"
                + str(column_three_width)
                + "}"
            )
...
class PyVerilator:
    ...
    def build(
        ...
        verilator_cpp_wrapper_path = os.path.join(
            build_dir, f"pyverilator_wrapper_{verilog_module_name}.cpp"
        )
        ...
        def search_for_signal_decl(signal_type, line):
            ...
            if signal_type in ("IN", "OUT", "SIG"):
                ...
                result = re.search(
                    r"/\*(\d+):(\d+)\*/"
                    + f" ({verilog_module_name})__{signal_type}__"
                    + r"([\w]+);",
                    line,
                )
            if result:
                ...
                if signal_type == "DOT":
                    ...
                    if result.group(3).startswith(verilog_module_name):
                        ...
                        return (
                            f"{result.group(3)}__{signal_type}__{result.group(4)}",
                            signal_width,
                        )
                    else:
                        return None
    ...
    def __init__(self, so_file, auto_eval=True, command_args=()):
        ...
        for sig_name in self.io:
            if (
                sig_name.lower() == "clock"
                or sig_name.lower() == "clk"
                or sig_name.startswith("i_clock")
                or sig_name.startswith("i_clk")
            ):
    ...
    def _read_embedded_data(self):
        self.module_name = ctypes.c_char_p.in_dll(
            self.lib, "_pyverilator_module_name"
        ).value.decode("ascii")

        # inputs
        num_inputs = ctypes.c_uint32.in_dll(self.lib, "_pyverilator_num_inputs").value
        input_names = (ctypes.c_char_p * num_inputs).in_dll(
            self.lib, "_pyverilator_inputs"
        )
        input_widths = (ctypes.c_uint32 * num_inputs).in_dll(
            self.lib, "_pyverilator_input_widths"
        )
        self.inputs = []
        ...
        output_names = (ctypes.c_char_p * num_outputs).in_dll(
            self.lib, "_pyverilator_outputs"
        )
        output_widths = (ctypes.c_uint32 * num_outputs).in_dll(
            self.lib, "_pyverilator_output_widths"
        )
        self.outputs = []
        ...
        num_internal_signals = ctypes.c_uint32.in_dll(
            self.lib, "_pyverilator_num_internal_signals"
        ).value
        internal_signal_names = (ctypes.c_char_p * num_internal_signals).in_dll(
            self.lib, "_pyverilator_internal_signals"
        )
        ...
        json_string = ctypes.c_char_p.in_dll(
            self.lib, "_pyverilator_json_data"
        ).value.decode("ascii")
    ...
    def set_vl_finish_callback(self, cb):
        ...
        self._lib_vl_finish_callback = (
            ctype_cb(lambda *args: cb(self, *args)) if cb else None
        )
        ...
        if self.gtkwave_active:
            ...
            if new_max_time > old_max_time:
                ...
                if window_end_time >= old_max_time and window_end_time < new_max_time:
                    ...
                    self.gtkwave_tcl.eval(
                        f"gtkwave::setWindowStartTime {(window_start_time + time_shift_amt)}"
                    )
    ...
    def start_gtkwave(self):
        ...
        warnings.filterwarnings(
            "ignore",
            r".*generated stderr message '\[[0-9]*\] start time.\\n\[[0-9]*\] end time.\\n'",
        )
        ...
        new_zf = zf - 5
        self.gtkwave_tcl.eval(f"gtkwave::setZoomFactor {new_zf}")
